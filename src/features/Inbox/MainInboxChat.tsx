...
function MainInboxChat({ chat: items, onSelectId }: Props) {
  return items.map((item, index) => {
    const latestChat = item.message_history.length-1;
    const latestMessage = item.message_history[latestChat].messages.length-1;
          ...
          <div className="flex flex-col text-left w-full gap-[.1rem]">
            ...
            <div className="text-primary-darkgray">
              ...
              <p
                ...
                title={
                  item.message_history[latestChat].messages[latestMessage].text!
                }
              >
                {item.message_history[latestChat].messages[latestMessage].text!}
              </p>
